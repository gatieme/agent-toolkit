...
| NA | [tt-a1i/archify](https://github.com/tt-a1i/archify) | 让 Claude 用自然语言描述生成精美的架构图、流程图、时序图、数据流图和状态生命周期图, 支持深色/浅色主题切换和多种图片格式导出, 基于 Cocoon-AI 的 architecture-diagram-generator fork 重写而成. 用户只需用大白话描述系统架构或流程, Claude 就会生成一个单文件 HTML, 内含精细渲染的技术图. |
| [ZeroZ-lab/gmdiagram]()
...
| handoff | [dazuiba/handoff](https://github.com/dazuiba/handoff) | 在 Claude Code / Codex 会话中直接把任务委托给 DeepSeek 或其他模型, 它最典型的用法是让 Claude 负责规划和审阅、DeepSeek 负责执行, 省钱的同时不损失战力.  DeepSeek 成跑腿的了. | NA | ⭐ | 65 |
...
| [](https://github.com/happydog-intj/luban-skill/blob/master/skills/linux-kernel.skill/SKILL.md) | Linux 内核编码风格 Skill. 蒸馏自 Linux 内核源码、Documentation/process/coding-style.rst、 Linus Torvalds 的代码审查邮件(LKML)、Greg KH 的贡献者指南.  触发词: 「Linux 内核风格」「像内核一样写 C」「kernel style」「写底层 C」.  适用: 系统级 C 代码、驱动、嵌入式、底层库开发. |
| [](https://atomgit.com/linux-dev/CVE-Agent) | Linux 内核 CVE 修复补丁自动回合 Agent |
...
| AI 味去除 | [](https://github.com/hylarucoder/ai-flavor-remover) | 1,054 |
| 说人话: 中文 AI 味清理 skill | [](https://github.com/MrGeDiao/shuorenhua) | 492 |
| 写稿 Agent | [dongbeixiaohuo/writing-agent](https://github.com/dongbeixiaohuo/writing-agent) | 280 |
| | [](https://github.com/Hello-SimpleAI/chatgpt-comparison-detection) | 1,382 |
| De-AI Prompt Enhancer & Writer Booster SKILL | [](https://github.com/OUBIGFA/De-AI-Prompt-Enhancer-Writer-Booster-SKILL) | 452 |
...
[2026/06/18, Mr Panda @PandaTalk8, 如何沉淀 SKILL: 把重复劳动变成可复用的能力 ](https://x.com/PandaTalk8/status/2067534592416915618)
...
| YAO(Yielding AI Outcomes) Meta Skills | [](https://github.com/yaojingang/yao-meta-skill) | 姚金刚老师把元Skill(yao-meta-skill), 把一次性 Prompt 变成可运营的技能资产. 把 AI Agent 的技能开发, 从「写 prompt」真正升级成了「工程化」. 它不是又一个 prompt 工具, 而是完整的 Skill OS:<br>1. 用平台无关的 Skill IR, 把意图、触发条件、输入输出、边界、证据要求固化成语义合约<br>2. 一键编译输出到 OpenAI、Claude、Cursor、VS Code 等多目标<br>3. 内置 Output Eval Lab + Review Studio 2.0, 做证据驱动的发布门禁与盲评<br>4. 全生命周期治理: adoption drift 追踪、权限探针、操作手册、持续迭代闭环;<br>核心理念直击当前最大痛点: 「The goal is not to generate more prompt text, but to produce reusable AI assets and real operational outcomes.」 从 1.0 的「把工作流打包」进化到 2.0 的「建一个技能、测一个技能、包一个技能、管一个技能」. 想把你团队里反复出现的决策流程、研究流程、内容生成流程, 变成真正可复用、可传承、可审计的 Agent 能力？这个项目就是那套方法论 + 工具链. [2.0升级方案](https://doc.laoyao.cn/unyyhc), [2.0与1.0的对比报告](https://doc.laoyao.cn/l3lkgm) | NA | ⭐ | 1,537 |
...
| 达尔文.skill | [alchaincyf/darwin-skill](https://github.com/alchaincyf/darwin-skill) | 达尔文.skill 2.0, 像训练模型一样优化你的 Agent Skills. 受 Andrej Karpathy 的 autoresearch 启发, 将自主实验循环从模型训练搬到 Skill 优化领域. 一个只能向前转的棘轮. v2.0 更新于 2026-05-28, 吸收微软研究院 [2026/05/22, SkillLens: From Raw Experience to Skill Consumption: A Systematic Study of Model-Generated Agent Skills](https://arxiv.org/abs/2605.23899)(研究skill应该怎么被评估) 与 [2026/05/22, SkillOpt: Executive Strategy for Self-Evolving Agent Skills](https://arxiv.org/abs/2605.23904)(研究skill应该怎么被优化) 两篇论文做的系统性升级. 参见 [2026/05/28, 花叔 @AlchainHust, 达尔文.skill 2.0正式开源发布！让你的所有skill左脚踩右脚实现自我进化](https://x.com/AlchainHust/status/2059913202507633024) | Claude Code | ⭐ | 4,284 |
...
| 鲁班 | [鲁班 | Luban](https://github.com/LearnPrompt/luban-skill) | [2026/06/14, 卡尔的AI沃茨 @aiwarts, 开源「鲁班」Skill, Claude Fable 5下线前跑通的升级工作流！](https://x.com/aiwarts/status/2066146382327484484)
| [](https://github.com/happydog-intj/luban-skill) | 鲁班是中国古代最传奇的工匠, 木工、建筑、机械的祖师爷. 鲁班.skill 用同样的逻辑做代码: 提炼任何代码库的编码 DNA, 生成可运行的风格 Skill, 让 AI 在写新代码时自动对齐目标风格. |
...
| 东方财富 SKILL | [dfcfs/finskillshub](https://marketing.dfcfs.com/views/finskillshub/index) | 有粉丝在通达信 MCP 那篇文章下面跟我说东方财富有 SKILL, 我搜一下了, 确实有, 还有每天 API KEY 领取, 大家赶紧薅个羊毛. |
| 通达信 | [通达信 Skill](https://vip.tdx.com.cn/site/app/pc-mall/main.html#/page_product_mcp) | 发现还是有很多人找不到实时的A股数据, 有的要么数据很贵, 要么数据延迟, 通达信很有格局, 直接将自家的数据打包成MCP, 可以直接通过AI调用, 后期我可以让AI帮我盯板块, 很多我忽视的概念板块也可以用到了 |
...
| [](https://github.com/baileyh8/hermes-feishu-streaming-card) | 340 |
| [](https://github.com/yangchuansheng/visual-ip-illustrations) | 119 |
...
| [himself65/finance-skills](https://github.com/himself65/finance-skills) | 一个为金融分析和交易提供的代理技能集合, 包含多种金融相关功能. 核心功能: 期权收益曲线图表生成(支持多种期权策略)、股票相关性分析(发现相关公司和配对交易 candidate)、金融数据获取(通过yfinance获取股票价格、财务报表等)、霍尔木兹海峡实时监控(航运、油价影响等)、社交媒体金融信息读取(Discord、Telegram、Twitter). 技术上遵循Agent Skills开放标准, 支持多种平台(Claude.ai、Claude Code等), 模块化设计, 支持交互式UI. 适用于金融分析、交易策略制定、市场情绪分析、地缘政治风险监控、投资决策支持等场景. [作者整理的 skills 集合](https://skills.himself65.com)  | ⭐⭐⭐ |
...
| [`RefoundAI/lenny-skills`](https://github.com/RefoundAI/lenny-skills) | 86个从 Lenny's Podcast 中提炼的产品管理技能, 由世界顶级产品领导者(包括 Shreyas Doshi、Marty Cagan、Elena Verna 等 90+位嘉宾)的智慧结晶, 专为产品经理、创始人和产品团队打造, 让 Claude 能协助完成真实产品工作. 目标: 将 100+ 期播客中的产品管理精华转化为 AI 代理可用的技能. 技术: 86 个 Markdown 格式的技能文件, 覆盖招聘与团队建设、用户研究与发现、战略与规划、交付与执行、领导力与协作、增长与变现、销售与上市、职业发展、AI 与技术等 9 大分类, 包括 writing-prds(写PRD)、evaluating-candidates(评估候选人)、stakeholder-alignment(利益相关者对齐)、prioritizing-roadmap(路线图优先级)等实用技能, 支持 CLI 一键安装. 使用场景: 撰写 PRD、评估候选人、设计产品策略、制定产品愿景、管理技术债务、运行有效会议、进行用户访谈、分析用户反馈、优化增长循环、设计定价策略、提升留存率、处理利益相关者关系等产品管理全流程.  | ⭐⭐⭐⭐ |
| [iamzifei/show-me-the-money](https://github.com/iamzifei/show-me-the-money) | 381 |
